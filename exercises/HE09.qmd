...
In this section, we will learn how to plot sparklines busing ggplot2.
...
The code chunk below is used to create sparklines.
...
The code chunk below is used to prepare a bullet chart report by using functions of gt and gtExtras packages.
...
First, we calculate summary statistics using the code chunk below.
...
Next, we use the code chunk below to add the statistics on the table.
...
Similarly, we can combine the bullet chart and sparklines using the code chunk below.
...
By default the pagesize is 10. In the code chunk below, arguments defaultPageSize is used to change the default setting so we can fit all 13 rows in one page.
...
Instead of adding the reference line, bandline can be added by using the `bandline` argument.
