...
subtitle: "Visualising and Analysing Geographical Data: Choropleth Mapping with R"
...
In this hands-on exercise, the key R package we will use is [**tmap**](https://cran.r-project.org/web/packages/tmap/) package. Beside **tmap** package, four other R packages will be used. They are:
...
The easiest and quickest way to draw a choropleth map using **tmap** is by using *qtm()*. It is concise and provides a good default visualisation in many cases.

FYI: **QTM = Quick Thematic Map plot**
...
Despite how useful it is for drawing a choropleth map quickly and easily, the disadvantge of *qtm()* is that it makes aesthetics of individual layers harder to control.
...
pretty <- tm_shape(mpsz_pop2020)+
  tm_polygons("DEPENDENCY",
      fill.scale = tm_scale_intervals(
        ...
        n = 5)) +
  tm_borders(fill_alpha = 0.5) +
  tm_layout(title = "Pretty classification",
...
kmeans <- tm_shape(mpsz_pop2020)+
  tm_polygons("DEPENDENCY",
      fill.scale = tm_scale_intervals(
        ...
        n = 5)) +
  tm_borders(fill_alpha = 0.5) +
  tm_layout(title = "Kmeans classification",
            title.position = c("center", "top"),
            frame = FALSE)

tmap_arrange(pretty, kmeans)
```
...
With reference to the results above, we set break points at 0.60, 0.70, 0.80, and 0.90. In addition, we also need to include a minimum and maximum, which we set at 0 and 100. Our *breaks* vector is thus c(0, 0.60, 0.70, 0.80, 0.90, 1.00).
...
Besides map style, **tmap** also also provides arguments to draw other map furniture such as compass, scale bar and grid lines.
...
In the below example, small multiple choropleth maps are created by assigning multiple values to at least one of the aesthetic arguments.
...
Instead of creating multiple small choropleth maps, you can also use selection function to map spatial objects meeting the selection criterion.
